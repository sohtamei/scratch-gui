var extName = 'LineTracer';			// _ などの記号厳禁
...
//*/
	...
	getInfo () {
		...
		return {
			id: extName,
			name: [extName,'ライントレーサ'][this._locale],
			menuIconURI: IconURI,   // Icon png to be displayed in the blocks category menu, encoded as a data URI.

			blocks: [
				{blockType: BlockType.COMMAND, opcode: 'setSpeed', text: ['speed','スピード'][this._locale]+'[ARG1]', arguments: {
	...
	setSpeed(args) {
		this.speed = args.ARG1*1;
	}

	setCar(args) {
		let dir = args.ARG1*1;
		const dir_table = [
			[{port:14,level:0}, {port:16,level:0},],	// 0:STOP
			[{port:14,level:0}, {port:16,level:0},],	// 1:FORWARD
			[{port:14,level:0}, {port:16,level:0},],	// 2:LEFT
			[{port:14,level:0}, {port:16,level:0},],	// 3:RIGHT
			[{port:14,level:1}, {port:16,level:1},],	// 4:BACK
			[{port:14,level:0}, {port:16,level:1},],	// 5:ROTATE LEFT
			[{port:14,level:1}, {port:16,level:0},],	// 6:ROTATE RIGHT
		];

		let pwm = (4096 * this.speed)/100;
		if(pwm > 4095) pwm = 4095;
		const pwm_table = [
			[{port:13,level:0},   {port:15,level:0},  ],	// 0:STOP
			[{port:13,level:pwm}, {port:15,level:pwm},],	// 1:FORWARD
			[{port:13,level:pwm}, {port:15,level:0},  ],	// 2:LEFT
			[{port:13,level:0},   {port:15,level:pwm},],	// 3:RIGHT
			[{port:13,level:pwm}, {port:15,level:pwm},],	// 4:BACK
			[{port:13,level:pwm}, {port:15,level:pwm},],	// 5:ROTATE LEFT
			[{port:13,level:pwm}, {port:15,level:pwm},],	// 6:ROTATE RIGHT
		];
		if(dir >= dir_table.length) return;

		const _this = this;
		if(dir == 0) {
			return _this.runtime.dev.comlib.setPwms(pwm_table[dir])
		} else {
			return _this.runtime.dev.comlib.digiWrite(dir_table[dir])
			.then(() => _this.runtime.dev.comlib.setPwms(pwm_table[dir]))
		}
	}
